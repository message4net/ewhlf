...
define('DIR_BASE', dirname(dirname(dirname(__DIR__))).DS);

$content=file_get_contents(DIR_BASE.'comm/init.cfg');

preg_match_all('/PHP[ ]+(.*)/', $content,$arrContent);
$len=count($arrContent[0]);
for($i=0;$i<$len;$i++){
	//echo preg_replace('/^PHP[ ]+/', '', $arrContent[0][$i]).'<br/>';
	eval(preg_replace('/^PHP[ ]+/', '', $arrContent[0][$i]));
}

require DIR_BASE.NAME_PHP.DS.NAME_COMM.DS.NAME_CFG.DS.NAME_SELF.JN.NAME_PHP;
require DIR_BASE.NAME_PHP.DS.NAME_COMM.DS.NAME_CFG.DS.NAME_REL.JN.NAME_PHP;

require DIR_BASE.NAME_PHP.DS.NAME_COMM.DS.NAME_INC.DS.NAME_INIT.JN.NAME_PHP;
require DIR_BASE.NAME_PHP.DS.NAME_COMM.DS.NAME_INC.DS.NAME_DBPDO.JN.NAME_PHP;
...
//define('NAME_PHP', 'php');
//
//require BASE_DIR.NAME_PHP.DS.NAME_COMM.DS.NAME_CFG.DS.'self.cfg.php';
//require BASE_DIR.NAME_PHP.DS.NAME_COMM.DS.NAME_CFG.DS.'rel.cfg.php';
//require BASE_DIR.NAME_PHP.DS.NAME_COMM.DS.NAME_INC.DS.'init.inc.php';
//require BASE_DIR.NAME_PHP.DS.NAME_COMM.DS.NAME_INC.DS.'dbpdo.inc.php';
//
//$APP_ARR=array(
//    'yonderwon',
//    '404'
